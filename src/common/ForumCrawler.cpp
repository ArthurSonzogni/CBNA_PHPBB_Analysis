...
ForumCrawler::ForumCrawler(const std::string& website,
    ...
    : website(website), save_dir(save_dir) {}
...
bool ForumCrawler::parse_users() {
  ...
  while (true) {
    ...
    if (user_parser.parse(user_id)) {
      ...
      std::cout << "Fail user = " << user_id << "" << std::endl;
      fail_allowed--;
      if (fail_allowed <= 0)
        break;
    }
...
void ForumCrawler::load_topics() {
  for (auto& file_name : list_files(save_dir + "/topics"))
    try {
...
void ForumCrawler::load_users() {
  for (auto& file_name : list_files(save_dir + "/users"))
    try {
      int id = std::stoi(file_name);
